...
class BertEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        if self.bert_name == "bert-base-uncased":
            bertpath = '/.t/pretrained_models/bert_base_uncased/'
            config = BertConfig.from_pretrained(bertpath)
            config.gradient_checkpointing = self.cfg.MODEL.LANGUAGE_BACKBONE.USE_CHECKPOINT
            self.model = BertModel.from_pretrained(bertpath, add_pooling_layer=False, config=config)
