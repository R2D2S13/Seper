...
class OKVQADataset_V2(Dataset):
    def __init__(self,
                 data_path="/.s/coco2014/",
                 json_path='/.do/data/sample_data/code_vqa/cache/okvqa/annotations/vqa_val_eval.json',
                 start_sample=0,
                 image_transforms=None, question_transforms=None, tokenize=None,
                 answer_selection=most_common_from_dict,
                 # answer_selection=all_answers_from_dict,
        ...
        df['sample_id'] = df['question_id']
        # Annotations
        df["mc_answer"] = df.answer.apply(most_common_from_dict)
        self.df = df
    ...
    def __getitem__(self, index):
        ...
        selected_answers = self.answer_selection(answer_list)  # Apply answer_selection() function to list of dict
        # Load and transform image
        with open(image_path, "rb") as f:
            pil_img = Image.open(f).convert("RGB")
        if self.image_transforms:
        ...
        # Return
            ...
            return {"sample_id": question_id,'image_name':image_path,'answer': selected_answers, "image": img, "query": question,
                    'pil_img': pil_img, 'index': index, 'possible_answers': [], 'info_to_prompt': question,
                    "question_type": -1}
    ...
    def get_item_score(self, p, g):
        g = [self.post_process(g_,stem=False) for g_ in g]
        p = self.post_process(p,stem=False)
        ...
        for i, g_ in enumerate(g):
            ...
            # for g_ in g:
            #     other_answers = [item for item in g if item != g_]
            matching_answers = [item for item in other_answers if item == p or item in p] # or item in p]
            acc = min(1., float(len(matching_answers)) / 3)
            accs.append(acc)
        item_score = sum(accs) / len(accs)
        return item_score
