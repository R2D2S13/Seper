...
def init_save_path(tags,timestamp):
    save_path = f'results/viper_gqa_sample200_{"_".join(tags)}'
    save_path=os.path.join(save_path,timestamp)
    return save_path

def init_plan_wrapper_mp(save_dir,log_dir,process_id,queues):
    ...
    plan_wrapper._init_varpool['ImagePatch'] = image_patch_partial
    plan_wrapper._init_varpool['llm_query'] = llm_query_partial
    return plan_wrapper

def worker_init(queues_in_,queue_results_,usable_keys_,using_keys_,save_dir,log_dir):
    global plan_wrapper,queue_results,queues_in,usable_keys,using_keys
    ...
    plan_wrapper = init_plan_wrapper_mp(save_dir,log_dir,index_queue,queues)
...
def test_task(query,image_path,image,sample_id,label):
    ...
    try:
        state,res = plan_wrapper.forward(query = query,
...
if __name__ == "__main__":
    ...
    dataset = MyDataset('gqa_train_sample_200')
