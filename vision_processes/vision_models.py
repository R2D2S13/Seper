...
import groundingdino.datasets.transforms as GDtransform
from torchvision.ops import box_convert
import numpy as np
from configs import config
import time
from copy import deepcopy
from LLM.chatgpt_function_model import chat_completion_request
...
class DepthEstimationModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, model_type='DPT_Large'):
        ...
        depth_estimation_model = hub.load('intel-isl/MiDaS', model_type, pretrained=True, skip_validation=True).to(self.dev)
...
class GroundingDinoModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, box_threshold=config.detect_thresholds.GD_BOX, text_threshold=config.detect_thresholds.GD_TEXT):
        BaseModel.__init__(self, gpu_number)
        from groundingdino.util.inference import load_model
        model = load_model(config.GD.config_path,config.GD.weight_path)
        model.to(self.dev)
        model.eval()
        ...
        self.nms_threshshold = 0.2

    @torch.no_grad()
    def forward(self, image: torch.Tensor, obj: Union[str, list],return_labels: bool = False,return_boxes1:bool = False):
        transform = GDtransform.Compose(
            [
                GDtransform.RandomResize([800], max_size=1333),
                GDtransform.ToTensor(),
                GDtransform.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
            ]
        )
        image_source = transforms.ToPILImage()(image).convert("RGB")
        image = np.asarray(image_source)
        image_transformed, _ = transform(image_source, None)

        boxes0, logits, phrases = predict(
            model=self.model,
        ...
        h, w, _ = image.shape
        boxes = boxes0 * torch.Tensor([w, h, w, h])
        boxes1 = box_convert(boxes=boxes, in_fmt="cxcywh", out_fmt="xyxy")
        selected_idx = nms(boxes1,logits,self.nms_threshshold)
        boxes1 = boxes1[selected_idx].numpy()
        ...
        if return_labels:
            return boxes2, boxes0, logits, phrases, image
        if return_boxes1:
            return boxes1
...
class InstructBlipModel(BaseModel):
    name = 'IBLIP'
    def __init__(self, gpu_number=0,model_path = None):
        from transformers import InstructBlipForConditionalGeneration,InstructBlipProcessor
        from torchvision.transforms import ToPILImage
        super().__init__(gpu_number)
        if model_path is None:
            instructp_blip_path = config.IBLIP.weight_path
        else:
            instructp_blip_path = model_path
        model = InstructBlipForConditionalGeneration.from_pretrained(instructp_blip_path,torch_dtype=torch.float16)
        processor = InstructBlipProcessor.from_pretrained(instructp_blip_path)
        model.to(self.dev)
        self.model = model
        self.processor = processor
        self.converter = ToPILImage()
        #design for image caption
        self.prompt = 'Describe the image in detail.'

    def generate_text(self,image,prompt=None,**kw):
    ...
    @torch.no_grad()
    def forward(self,image,prompt,**kw):
        ...
        res = self.generate_text(image,prompt,**kw)
        return res
    
class GroundingSamModel(BaseModel):
    name = 'GDSAM'
    def __init__(self, gpu_number=0, box_threshold=config.detect_thresholds.GD_BOX, text_threshold=config.detect_thresholds.GD_TEXT):
        BaseModel.__init__(self, gpu_number)
        from groundingdino.util.inference import load_model
        ...
        grounding_model = load_model(config.GD.config_path,config.GD.weight_path)
        grounding_model.to(self.dev)
        grounding_model.eval()

        sam_checkpoint =config.SAM.weight_path
    ...
    @torch.no_grad()
    def forward(self, image: torch.Tensor, obj: Union[str, list]):
        transform = GDtransform.Compose(
            [
                GDtransform.RandomResize([800], max_size=1333),
                GDtransform.ToTensor(),
        ...
        h, w, _ = image.shape
        boxes = boxes0 * torch.Tensor([w, h, w, h])
        boxes_xy = box_convert(boxes=boxes, in_fmt="cxcywh", out_fmt="xyxy").numpy()
        transformed_boxes = self.sam_predictor.transform.apply_boxes_torch(torch.tensor(boxes_xy),(h,w)).to(self.dev)
        ...
        coordinates = deepcopy(boxes_xy)
        # Return [left, lower, right, upper] instead of [left, upper, right, lower]
        coordinates[:, 1] = h - boxes_xy[:, 3]
        coordinates[:, 3] = h - boxes_xy[:, 1]
        
        return coordinates,boxes_xy,masks.cpu().numpy(),image
...
class OwlViTModel_V2(BaseModel):
    name = 'owlvit_visprog'
    ...
    def __init__(self, 
        ...
        from transformers import OwlViTProcessor, OwlViTForObjectDetection
        if config.owlvit_visprog.weight_path is None:
            weight_path = 'google/owlvit-large-patch14'
        else:
            weight_path = config.owlvit_visprog.weight_path
        processor = OwlViTProcessor.from_pretrained(weight_path)
        model = OwlViTForObjectDetection.from_pretrained(weight_path)
        model.eval()
        model.requires_grad_(False)

        threshold=config.owlvit_visprog.thresh,
        nms_thresh=config.owlvit_visprog.nms_thresh
    ...
    @torch.no_grad()
    def forward(self,img,obj_name):
        encoding = self.processor(
            text=[[f'a photo of {obj_name}']], 
            images=img,
            return_tensors='pt')
        encoding = {k:v.to(self.dev) for k,v in encoding.items()}
        with torch.no_grad():
        ...
        selected_boxes, selected_scores = self.nms(
...
class TCLModel(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        text_encoder = '/.t/pretrained_models/bert_base_uncased/'
        checkpoint_path = '/.t/pretrained_models/TCL/TCL_4M.pth'
...
class GPT3Model(BaseModel):
    ...
    def query_gpt3(self, prompt, model="text-davinci-003", max_tokens=30, logprobs=None, stream=False,
                   stop=None, top_p=1, frequency_penalty=0, presence_penalty=0):
        if not isinstance(prompt,list):
            prompt=[prompt]
        messages = [{"role": "user", "content": p} for p in prompt]
        if openai.api_key is not None and not config.key_pooling:
            ...
            use_random_key = True
            key = ''
        response = chat_completion_request(key=key,
                                           random_key=use_random_key,
                                           messages=messages,
    ...
    @classmethod
    def list_processes(cls):
        return ['gpt3_' + n for n in ['qa', 'general','guess']]
...
def codex_helper(extended_prompt):
    ...
    if config.codex.model in ("gpt-4", "gpt-3.5-turbo"):
        ...
        responses = [response_with_timelimit(prompt) for prompt in extended_prompt]
...
def response_with_timelimit(prompt,time_limit=20):
...
class BLIPModel(BaseModel):
    ...
    to_batch = True
    max_batch_size = 32
    seconds_collect_data = 0.5  # The queue has additionally the time it is executing the previous forward pass
    ...
    def __init__(self, gpu_number=0):
        ...
        if  blip_path is None:
            if blip_v2_model_type is not None:
                blip_path='Salesforce/{blip_v2_model_type}'
            else:
                raise EnvironmentError(f"You must indicate blip2_path or blip2_type")
        
        self.processor = Blip2Processor.from_pretrained(blip_path)
        # Device_map must be sequential for manual GPU selection
        try:
            self.model = Blip2ForConditionalGeneration.from_pretrained(
                blip_path, load_in_8bit=half_precision,torch_dtype=torch.float16,
                device_map="sequential", max_memory=max_memory
            )
            if blip_v2_model_type=='blip2-lora':
                from peft import PeftModel
                self.model=PeftModel.from_pretrained(self.model,'/.Tplus/vipergptplus/blip_vqa_adapter')
        except Exception as e:
    ...
    def caption(self, image, prompt=None):
        ...
        generated_ids = self.model.generate(**inputs, length_penalty=1., num_beams=2, max_length=30, min_length=1,
...
class BLIPModel_V2(BaseModel):
    name = 'blip1_visprog'
    def __init__(self,gpu_number=0):
        super().__init__(gpu_number)
        from transformers import AutoProcessor,BlipForQuestionAnswering
        if config.blip1_visprog.weight_path is not None:
            weight_path = config.blip1_visprog.weight_path
        else:
            weight_path = "Salesforce/blip-vqa-capfilt-large"
        self.processor = AutoProcessor.from_pretrained(weight_path)
        self.model = BlipForQuestionAnswering.from_pretrained(
            weight_path).to(self.dev)
        self.model.eval()
...
class XVLMModel(BaseModel):
    ...
    def __init__(self, gpu_number=0,
        ...
        self.model = model
        self.tokenizer = BertTokenizer.from_pretrained('/.ngdinoForViper/bert-base-uncased/')
