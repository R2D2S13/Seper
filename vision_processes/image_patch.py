...
from utils.utils import show_single_image, load_json
from vision_processes.vision_processes import forward, config
...
class ImagePatch:
    ...
    # def locate(self,direction:Literal['right','left','upper','bottom']):
    ...
    def find(self, object_name: str, Box_Threshold: int = None, return_labels: bool = False):
        ...
        """
        if config.load_models.GD and not config.load_models.GDSAM:
            if return_labels:
        ...
        elif config.load_models.GDSAM:
            all_object_coordinates, boxes, all_masks,ori_image_array= self.forward('GDSAM',self.cropped_image, object_name)
            all_masks = torch.tensor(all_masks)
            patches = []
            for coordinates,box,mask in zip(all_object_coordinates,boxes,all_masks):
                x1,y1,x2,y2 = int(box[0]),int(box[1]),int(box[2]),int(box[3])
                masks_array = mask[0].numpy()
                if (masks_array==True).any():
                    masks_array = np.expand_dims(masks_array,-1).repeat(3,axis=-1)
                    masks_array = masks_array.astype(np.uint8)*255
                    mask_image = cv2.bitwise_and(ori_image_array,masks_array)
                    crop_image = mask_image[y1:y2,x1:x2]
                    patch = self.crop(*coordinates)
                    patch.cropped_image = crop_image
                else:
                    patch = self.crop(*coordinates)
        ...
        if threshold > 0:
            ...
            mask = [i for i,m in enumerate(mask) if m]
            # if not mask.any():
    ...
    def get_property(self, object_name: str, attribute: str) -> str:
        ...
        """
        question = f'What is the {attribute} of {object_name}?'
    ...
    def simple_query(self, question: str):
        ...
        """
        model_name = config.simple_query.model
        print(f'simple_query_model_name{model_name}')
        if model_name == 'IBLIP':
            return self.forward(model_name, self.cropped_image, question)
        elif model_name == 'blip':
            return self.forward(model_name, self.cropped_image, question, task='qa')
    ...
    def llm_query(self, question: str, long_answer: bool = True) -> str:
        ...
        if self.caption is None:
            self.caption = self.simple_query(question='Describe the image in detail.')
        prompt = f'The image describe {self.caption}.' + question
        if long_answer:
            return self.forward(model_name='gpt3_general', prompt=prompt)
        else:
            return self.forward(model_name='gpt3_qa', prompt=[prompt])
...
def distance(patch_a: Union[ImagePatch, float], patch_b: Union[ImagePatch, float]) -> float:
    ...
    if isinstance(patch_a, float) and isinstance(patch_b, float):
